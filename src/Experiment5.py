...
from ModelConstruct import Basic_GRU, Encoder2, Decoder, MLPDecoder, EncoderDecoder
...
if __name__ == '__main__':
    ...
    model.encoder.load_state_dict(torch.load(os.path.realpath(os.path.join(os.path.dirname(__file__),
                                                                     '..',
                                                                     'CHECKPOINTS',
                                                                     'Experiment15encoder.pt'))))
    model.decoder.load_state_dict(torch.load(os.path.realpath(os.path.join(os.path.dirname(__file__),
                                                                     '..',
                                                                     'CHECKPOINTS',
                                                                     'Experiment15encoder.pt'))))
    ...
    saved_model.eval()
    output_list, enc_output_list, label_list = Helper.encdec_predict_test4(saved_model, test_loader, pred_step, num_layers, hidden_dim)
